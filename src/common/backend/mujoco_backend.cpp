...
hardware_interface::CallbackReturn MujocoBackend::init(const hardware_interface::HardwareInfo& info,
    ...
    if (!unstable_) {
        for (const auto& name : joint_names_) {
            ...
            if (std::abs(data->qvel[dof_idx]) > 100.0 || std::isnan(data->qvel[dof_idx])) {
                unstable_ = true;
                RCLCPP_FATAL(logger_, "=== INSTABILITY at tick %d, t=%.6f, joint '%s' ===", control_tick_, data->time,
                             name.c_str());
                for (int j = 0; j < model->njnt; ++j) {
                    const char* jname = mj_id2name(model, mjOBJ_JOINT, j);
                    int q = model->jnt_qposadr[j];
                    int v = model->jnt_dofadr[j];
                    RCLCPP_FATAL(logger_, " joint '%s': qpos=%.6f qvel=%.6f", jname ? jname : "?", data->qpos[q],
                                 data->qvel[v]);
                }
                break;
            }
...
void MujocoBackend::setup_scene_tf_publisher() {
    ...
    scene_timer_ = node_->create_wall_timer(
        ...
        [this]() { publish_scene_tf(); }, scene_cb_group_);
