...
from brax.training import acting, distribution, gradients, pmap, types
...
def train(
    ...
    assert m_heads >= 1
    assert num_eval_envs % m_heads == 0, "num_eval_envs must be divisible by num_policy_heads"
    ...
    # --- multi-agent partition divisibility (per device) ---
    # Each device holds num_envs_local envs, split into m_heads contiguous blocks;
    # each head's rollout rows must split evenly into the PPO minibatches.
    num_envs_local = num_envs // device_count
    assert num_envs_local % m_heads == 0, "num_envs // device_count must be divisible by num_policy_heads"
    _rows_per_head = (batch_size * num_minibatches // num_envs) * (num_envs_local // m_heads)
    assert _rows_per_head % num_minibatches == 0, "rows per head must be divisible by num_minibatches"
    ...
    # --- multi-agent networks: ONE shared policy trunk + `m_heads` policy heads,
    #     and a single SHARED value network (only the policy is multi-head). ---
    # Env e is routed to head e // (num_envs // m_heads) — contiguous blocks, so
    # n/m envs share each head. Each head is trained on its own envs' experience;
    # the shared trunk receives gradients from all heads (sum of per-head losses).
    if isinstance(obs_shape, Mapping):
        raise NotImplementedError("multi_agent_disagreement_ppo supports flat (array) observations only.")
    obs_size_flat = int(np.prod(obs_shape))
    ...
    def _policy_apply(processor_params, policy_params, obs):
        # policy_params is a SINGLE head's params: {'trunk': ..., 'head': ...}.
        obs = normalize(obs, processor_params)
        feat = policy_trunk.apply(policy_params["trunk"], obs)
        return policy_head_module.apply(policy_params["head"], feat)

    def _policy_single_init(key):
        kt, kh = jax.random.split(key)
        dummy = jnp.zeros((1, obs_size_flat))
        tp = policy_trunk.init(kt, dummy)
        hp = policy_head_module.init(kh, policy_trunk.apply(tp, dummy))
        return {"trunk": tp, "head": hp}

    policy_network = base_networks.FeedForwardNetwork(init=_policy_single_init, apply=_policy_apply)
    value_network = base_networks.make_value_network(
        obs_size_flat,
    ...
    def make_multihead_policy(params, deterministic: bool = False):
        """Rollout/eval policy that routes env-block e→head e//(N/m)."""
        norm_p, policy_p, value_p = params

        def policy(observations, key_sample):
            n = observations.shape[0]
            per = n // m_heads
            obs_g = observations.reshape(m_heads, per, observations.shape[-1])

            def head_logits(head_p, obs_h):
                return _policy_apply(norm_p, {"trunk": policy_p["trunk"], "head": head_p}, obs_h)
            ...
            if compute_value:
                extras["value"] = value_network.apply(norm_p, value_p, observations)
            return actions, extras

        return policy

    make_policy = make_multihead_policy
    ...
    # --- multi-head PPO loss ---
    # The loss is the MEAN over heads of brax's compute_ppo_loss, each evaluated on
    # that head's own minibatch with that head's params and the SHARED trunk/value.
    # Differentiating w.r.t. the full param pytree gives the desired gradient
    # structure automatically: the shared trunk and shared value accumulate
    # gradients from ALL heads (all experience), while head h's params receive
    # gradients only from head h's data.
    def multihead_loss(params, normalizer_params, data, key):
        p_trunk = params.policy["trunk"]
        value_params = params.value
        keys = jax.random.split(key, m_heads)

        def head_loss(head_p, data_h, key_h):
            head_params = ppo_losses.PPONetworkParams(policy={"trunk": p_trunk, "head": head_p}, value=value_params)
            return loss_fn(head_params, normalizer_params, data_h, key_h)

        losses, metrics = jax.vmap(head_loss)(params.policy["heads"], data, keys)
        return jnp.mean(losses), jax.tree_util.tree_map(jnp.mean, metrics)

    multihead_loss_and_grad = gradients.loss_and_pgrad(multihead_loss, pmap_axis_name=_PMAP_AXIS_NAME, has_aux=True)
    ...
    def minibatch_step(
        carry,
        data: types.Transition,  # head-grouped: (m_heads, minibatch, T, ...)
        normalizer_params: running_statistics.RunningStatisticsState,
    ):
        optimizer_state, params, key = carry
        key, key_loss = jax.random.split(key)
        (_, metrics), grads = multihead_loss_and_grad(params, normalizer_params, data, key_loss)
    ...
    def sgd_step(
        carry,
        unused_t,
        data: types.Transition,  # head-grouped: (m_heads, rows_per_head, T, ...)
        normalizer_params: running_statistics.RunningStatisticsState,
    ):
        optimizer_state, params, key = carry
        key, key_perm, key_grad = jax.random.split(key, 3)
        rows_per_head = data.observation.shape[1]

        def convert_data(x: jnp.ndarray):
            # Shuffle each head's rows, then split into minibatches per head.
            x = jnp.take(x, jax.random.permutation(key_perm, rows_per_head), axis=1)
            x = jnp.reshape(x, (m_heads, num_minibatches, -1) + x.shape[2:])
            return jnp.swapaxes(x, 0, 1)  # (num_minibatches, m_heads, minibatch, ...)
    ...
    def training_step(
        ...
        # Group rollout rows by head before the per-head SGD.
        #   (B', T, ...) -> (m_heads, B'/m_heads, T, ...).
        # Row r = l*num_envs + e (l over unroll blocks); env e is in head
        # e // (num_envs // m_heads) — the same contiguous-block routing the rollout
        # policy uses, so each head trains on exactly its own envs' experience.
        n_unroll_blocks = batch_size * num_minibatches // num_envs
        per_head_envs = num_envs_local // m_heads

        def group_by_head(x):
            x = jnp.reshape(x, (n_unroll_blocks, num_envs_local) + x.shape[1:])
            x = jnp.reshape(x, (n_unroll_blocks, m_heads, per_head_envs) + x.shape[2:])
            x = jnp.swapaxes(x, 0, 1)
            return jnp.reshape(x, (m_heads, n_unroll_blocks * per_head_envs) + x.shape[3:])

        data_grouped = jax.tree_util.tree_map(group_by_head, data)

        (optimizer_state, params, _), metrics = jax.lax.scan(
            functools.partial(sgd_step, data=data_grouped, normalizer_params=normalizer_params),
        ...
        #   behavioral_diversity_cross_head — how differently the m heads act on the
        #                       SAME states: variance ACROSS heads of each head's mean
        #                       (deterministic) action on a common batch, meaned over
        #                       states + action dims. 0 at m=1; > 0 measures the policy
        #                       population's spread — the mechanism behind multi-head
        #                       exploration (vs action_dispersion, which pools all
        #                       envs and mixes within- and across-head variance).
        metrics["action_dispersion"] = jnp.mean(jnp.std(act_f, axis=0))
        # Push a common batch of states through every head (shared trunk, per-head
        ...
        metrics["behavioral_diversity_cross_head"] = jnp.mean(jnp.var(_head_actions, axis=0))
        if entropy_cost > 0:
    ...
    # Policy = ONE shared trunk + `m_heads` heads (stacked on a leading axis);
    # value = a single shared network.
    key_ptrunk, key_pheads = jax.random.split(key_policy)
    _dummy_obs = jnp.zeros((1, obs_size_flat))
    _ptrunk = policy_trunk.init(key_ptrunk, _dummy_obs)
    _pfeat = policy_trunk.apply(_ptrunk, _dummy_obs)
    _pheads = jax.vmap(lambda k: policy_head_module.init(k, _pfeat))(jax.random.split(key_pheads, m_heads))
    init_params = ppo_losses.PPONetworkParams(
        policy={"trunk": _ptrunk, "heads": _pheads},
